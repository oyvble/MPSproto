//This script contains functions for calculating the cumulative expressions used for model validation.
/*ABOUT:
...
-> Stores cumulative values for each allele (inner genotype sum is parallelized)
*/
...
void loglikPrediction_cumprob_GA(double *pvalVEC, double *maxY, int *nJointCombs, int *NOC, int *NOK, 
				...
				for (int iter = 0; iter < nJointCombs[locind]; iter++) { //for each combined/joint genotype outcome		
					...
					for (a = 0; a < nAlleles; a++) { //traverse each observed alleles (also consider the last allele since Q-allele may have PH last allee)
						for(r = 0; r < nRep0; r++) { //traverse each replicates (observed alleles indicated by PH)
							...
							if( aaind!=cind2 ) { //proceed as before if different index							
								...
								if(shapev2[a]>smalltol) { //If contribution and PH>0  					//CONTRIBUTION SET (A)
									...
									logevidProb += noiseSizeWeightLong2[ cind2 ]; //likelihood for observed dropin (this will be the cumulative expression)						
									nDropin[r] += 1; //count dropin for particular replicate											
...
void loglikPrediction_cumprob_NB(double *pvalVEC, double *maxY, int *nJointCombs, int *NOC, int *NOK, 
	double *mixprop, double *mu, double *omega, double *beta, double *theta_Am, //eps is stutter proportion vector (1 element for each kind)
	double *AT, double *fst, double *nNoiseParam, double *noiseSizeWeightLong, double *noiseSizeWeightLong2, //note the additional weight 
   	int *nMarkers, int *nRep, int *nAllelesVEC, int *startIndMarker_nAlleles, int *startIndMarker_nAllelesReps,	
	double *peaksLong, double *peaksLong2, double *freqsLong, double *nTypedLong, double *maTypedLong, double *basepairLong,
						...
						for(r = 0; r < nRep0; r++) { //traverse each replicates (observed alleles indicated by PH)
							...
							if( aaind!=cind2 ) { //proceed as before if different index							
								...
								if(shapev2[a]>smalltol) { //If contribution and PH>0  					//CONTRIBUTION SET (A)
									...
									logevidProb += log( pnbinom_mu(peak, size0, shapev2[a], 1, 0) - pnbinom_mu(AT0-1,size0,shapev2[a],1,0) );									
									//logevidProb += log( pgamma(peak, shapev2[a],scale0, 1, 0) - pgamma(AT0-1, shapev2[a],scale0, 1, 0));
